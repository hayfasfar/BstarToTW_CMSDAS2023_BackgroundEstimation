...
period = 1
if '2018' in options.signals:
    period = 18
    options.lumi = '6.0'
elif '2017' in options.signals:
    period = 17
    options.lumi = '4.15'
elif '2016' in options.signals:
    period = 16
    options.lumi = '3.59'
...
if 'zprime' in options.signals:
    signal_string = "Z'"
    
    if 'DM' in options.signals:
        signal_string = "Z'_{DM}"
    elif '10' in options.signals:
        legend_string = "10% Width"
    elif '30' in options.signals:
        legend_string = "30% Width"
    
# elif options.signals == 'tt_signals_17.txt':
#     period = 17
#     options.lumi = '41.5'
# elif options.signals == 'tt_signals_16.txt':
#     period = 16
#     options.lumi = '35.9'
# elif options.signals == 'tt_signals_16APV.txt':
#     period = 16.2
#     options.lumi = '19.8'
...
if not options.blind:
    ...
    print 'Blinded'
    g_mclimit.GetXaxis().SetTitle("m_{"+signal_string+"_{"+cstr+"}} [TeV]")  # NOT GENERIC
    g_mclimit.GetYaxis().SetTitle("#sigma_{"+signal_string+"_{"+cstr+"}} #times B("+signal_string+"_{"+cstr+"}#rightarrow t #bar{t}) (pb)") # NOT GENERIC
...
# g_mclimit.SetLineWidth(3)
# g_mclimit.SetMarkerSize(0.)
# g_mclimit.GetXaxis().SetTitle("M_{"+signal_string+"} (TeV/c^{2})")
# g_mclimit.GetYaxis().SetTitle("Upper Limit #sigma_{"+signal_string+"_{"+cstr+"}} #times b (pb)")
...
if not options.blind:
    g_limit.GetXaxis().SetTitle("m_{"+signal_string+"_{"+cstr+"}} [TeV]")  # NOT GENERIC
    g_limit.GetYaxis().SetTitle("#sigma_{"+signal_string+"_{"+cstr+"}} #times B("+signal_string+"_{"+cstr+"}#rightarrow t #bar{t}) (pb)") # NOT GENERIC
...
legend.AddEntry(graphWP, "Theory "+signal_string+"_{"+cstr+"}"+" "+legend_string, "l")   # NOT GENERIC
# legend.AddEntry(graphWPup, "Theory "+signal_string+"_{"+cstr+"} 1 #sigma uncertainty", "l")
...
climits.SaveAs("limits/pdf/limits_combine_"+options.signals[options.signals.find('/')+1:options.signals.find('.')]+'_'+cstr+".pdf")
climits.SaveAs("limits/png/limits_combine_"+options.signals[options.signals.find('/')+1:options.signals.find('.')]+'_'+cstr+".png")
