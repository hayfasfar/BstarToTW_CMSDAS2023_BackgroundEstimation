...
def make_workspace():
    ...
    twoD = TwoDAlphabet('ttbarfits18_3x1', 'ttbar18.json', loadPrevious=False)
...
def ML_fit(signal):
    ...
    # the default workspace directory, created in make_workspace(), is called ttbarfits18_3x1/
    twoD = TwoDAlphabet('ttbarfits18_3x1', 'ttbar18.json', loadPrevious=True)
...
def GoF(signal, tf='', nToys=500, condor=False):
    ...
    fitDir = 'ttbarfits18_3x1{}'.format('_'+tf if tf != '' else '')
...
def plot_GoF(signal, tf='', condor=False):
    ...
    '''
    plot.plot_gof('ttbarfits18_3x1{}'.format('_'+tf if tf != '' else ''), '{}_area'.format(signal), condor=condor)
